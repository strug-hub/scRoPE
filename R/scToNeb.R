#' Retrieve data from Seurat or SingleCellExperiment object to prepare for use in scRoPE
#'
...
#' @return data_neb: A list usable for scRoPE.
#' @export
#' @examples
#' \dontrun{
#' library(Seurat)
...
scToNeb <- function(obj, assay = NULL, id = NULL, pred = NULL, offset = NULL, verbose = TRUE)
{
  ...
  if ("SingleCellExperiment" %in% class(obj)) {
    if (!requireNamespace("SingleCellExperiment", quietly = TRUE)) {
      stop("Package 'SingleCellExperiment' is required for SingleCellExperiment inputs.")
    }
    ...
    if (!is.null(id)) {
      ...
      warning("No sample ID provided, please specify it when running scRoPE.")
    }
    if (!is.null(offset)) {
      if(offset %in% covs)
    ...
    if (!requireNamespace("Seurat", quietly = TRUE)) {
      stop("Package 'Seurat' is required for Seurat inputs.")
    }
    if (is.null(assay)) {
